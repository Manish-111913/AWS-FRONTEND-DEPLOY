...
const CreateReorder = ({ onNavigateBack, onNavigateToVendor, selectedVendor, onClearSelectedVendor }) => {
    ...
    const handleCreateReorder = async () => {
        ...
        try {
            ...
            // =================== SIMPLE MULTI-VENDOR MESSAGING (NO CATEGORIZATION) ===================
            // Send order messages to ALL preferred vendors without category filtering
            
            console.log('\n🚀 SIMPLE MULTI-VENDOR ORDER DISTRIBUTION');
            ...
            // Display preferred vendors list
            console.log('\n📋 PREFERRED VENDORS LIST:');
            preferredVendors.forEach((vendor, index) => {
                console.log(`   ${index + 1}. ${vendor.name} (${vendor.vendor_category || 'No category'}) - WhatsApp: ${vendor.whatsapp_number || 'Not provided'}`);
            });

            // Simple approach: Send the SAME order to ALL preferred vendors
            console.log('\n📤 SENDING ORDERS TO ALL PREFERRED VENDORS...');
            const orderResults = [];
            let totalOrderCount = 0;

            // Create and send orders to each preferred vendor
            for (let i = 0; i < preferredVendors.length; i++) {
                const vendor = preferredVendors[i];
                
                console.log(`\n� ${i + 1}/${preferredVendors.length} - Creating order for: ${vendor.name}`);
                console.log(`   WhatsApp: ${vendor.whatsapp_number || 'Not provided'}`);
                console.log(`   Items: ${selectedItems.map(item => item.name).join(', ')}`);
            ...
            // Success! Show comprehensive success modal
            console.log(`\n🎉 MULTI-VENDOR ORDER COMPLETE!`);
            console.log(`   ✅ ${orderResults.length}/${preferredVendors.length} vendors received orders`);
            console.log(`   📱 WhatsApp messages sent to all vendors`);
            
            orderResults.forEach((result, index) => {
                console.log(`   ${index + 1}. ${result.vendor} - PO: ${result.poNumber} - WhatsApp: ${result.whatsappStatus?.success ? '✅' : '❌'}`);
            });

            // Success! Show comprehensive success modal
            setSuccessInfo({
                poNumber: orderResults.map(r => r.poNumber).join(', '),
                totalAmount: 'Multiple Orders',
                itemCount: selectedItems.length,
            ...
            // Enable button immediately after successful order creation
            setCreating(false);

        } catch (e) {
            console.error('Error creating reorder:', e);
            setError(e.message);
            setCreating(false); // Also enable button on error
        }
    };
